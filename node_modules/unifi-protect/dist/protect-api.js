...
export class ProtectApi extends EventEmitter {
    ...
    checkAdminUserStatus(isFirstRun = false) {
        ...
        // Find this user.
        const user = this._bootstrap?.users.find((x) => x.id === this._bootstrap?.authUserId);
     ...
     * @category API Access
     */
    async getSnapshot(device, width, height, timestamp = Date.now(), usePackageCamera = false) {
            ...
            if (error instanceof FetchError) {
                switch (error.code) {
                    ...
                    case "ECONNRESET":
                        ...
                        break;
                    case "ENOTFOUND":
                        this.log.error("Hostname or IP address not found: %s. Please ensure the address you configured for this UniFi Protect controller is correct.", this.nvrAddress);
                        break;
                    default:
