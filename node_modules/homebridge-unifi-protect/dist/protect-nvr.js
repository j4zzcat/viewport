...
import { ProtectCamera, ProtectChime, ProtectDoorbell, ProtectLight, ProtectLiveviews, ProtectNvrSystemInfo, ProtectSensor, ProtectViewer } from "./devices/index.js";
...
export class ProtectNvr {
    ...
    constructor(platform, nvrOptions) {
        ...
        this.log = {
            debug: (message, ...parameters) => this.platform.debug(util.format((this.ufpApi?.name ?? this.name) + ": " + message, ...parameters)),
            error: (message, ...parameters) => this.platform.log.error(util.format((this.ufpApi?.name ?? this.name) + ": " + message, ...parameters)),
            info: (message, ...parameters) => this.platform.log.info(util.format((this.ufpApi?.name ?? this.name) + ": " + message, ...parameters)),
            warn: (message, ...parameters) => this.platform.log.warn(util.format((this.ufpApi?.name ?? this.name) + ": " + message, ...parameters))
        };
    ...
    async login() {
        ...
        for (const device of [this.ufp, ...bootstrap.cameras, ...bootstrap.chimes, ...bootstrap.lights, ...bootstrap.sensors, ...bootstrap.viewers]) {
            ...
            if ((device.modelKey !== "nvr") &&
                (device.isAdoptedByOther || device.isAdopting || !device.isAdopted)) {
                continue;
            }
    ...
    addProtectDevice(accessory, device) {
        ...
        switch (device.modelKey) {
            ...
            case "chime":
                ...
                this.configuredDevices[accessory.UUID] = new ProtectChime(this, device, accessory);
                return true;
            case "light":
    ...
    addHomeKitDevice(device) {
        ...
        if ((accessory = this.platform.accessories.find(x => x.UUID === uuid)) === undefined) {
            ...
            this.log.info("%s: Adding %s to HomeKit%s.", this.ufpApi.getDeviceName(device), device.modelKey, this.hasFeature("Device.Standalone", device) ? " as a standalone device" : "");
    ...
    discoverAndSyncAccessories() {
        ...
        ProtectDeviceCategories.map(category => this.ufpApi.bootstrap && this.ufpApi.bootstrap[category + "s"] &&
            (this.ufpApi.bootstrap[category + "s"] ?? []).map(device => this.addHomeKitDevice(device)));
        ...
        for (const accessory of this.platform.accessories.filter(x => x.context.nvr === this.ufp.mac)) {
            ...
            if (!protectDevice) {
                ...
                if (!("mac" in accessory.context)) {
                    ...
                    if (accessory.context.packageCamera.length) {
                        const uuid = this.hap.uuid.generate(accessory.context.packageCamera);
                ...
                const device = ProtectDeviceCategories.map(category => this.ufpApi.bootstrap && this.ufpApi.bootstrap[category + "s"] &&
                    (this.ufpApi.bootstrap[category + "s"] ?? []).find(x => x.mac === accessory.context.mac)).reduce((acc, x) => x ?? acc, null);
                this.log.info("%s: Removing %s from HomeKit.%s", device ? this.ufpApi.getDeviceName(device) : accessory.displayName, device ? device.modelKey : "device", accessory._associatedHAPAccessory.bridged ? "" : " You will need to manually delete the device in the Home app to complete the removal.");
            ...
            if (this.ufpApi.bootstrap[protectDevice.ufp.modelKey + "s"]?.some(x => x.mac === protectDevice.ufp.mac) &&
    ...
    removeHomeKitDevice(protectDevice) {
        ...
        if ("packageCamera" in protectDevice.accessory.context) {
            const uuid = this.hap.uuid.generate(protectDevice.accessory.context.packageCamera);
        ...
        if ((delayInterval !== undefined) && (delayInterval > 0)) {
            ...
            if (!this.deviceRemovalQueue[protectDevice.accessory.UUID]) {
                ...
                this.log.info("%s: Delaying device removal for %s second%s.", protectDevice.ufp.name ? this.ufpApi.getDeviceName(protectDevice.ufp) : protectDevice.accessoryName, delayInterval, delayInterval > 1 ? "s" : "");
                return;
            }
            // Is it time to process this device removal?
        ...
        // Remove this device.
        this.log.info("%s: Removing %s from HomeKit.%s", protectDevice.ufp.name ? this.ufpApi.getDeviceName(protectDevice.ufp) : protectDevice.accessoryName, protectDevice.ufp.modelKey ? protectDevice.ufp.modelKey : "device", protectDevice.accessory._associatedHAPAccessory.bridged ? "" : " You will need to manually delete the device in the Home app to complete the removal.");
        ...
        if (protectDevice.ufp.modelKey === "camera") {
            const protectDoorbell = protectDevice;
        ...
        server.on("request", (request, response) => {
            ...
            if (this.ufpApi.bootstrap) {
                ...
                for (const camera of this.ufpApi.bootstrap.cameras.filter(x => x.channels.some(channel => channel.isRtspEnabled)).sort((a, b) => {
                    ...
                    const publishEntry = (name = camera.name, description = "camera", rtspAlias = camera.channels[0].rtspAlias) => {
                        response.write(util.format("#EXTINF:0 channel-id=\"%s\" tvc-stream-vcodec=\"h264\" tvc-stream-acodec=\"opus\" tvg-logo=\"%s\" ", name, "https://raw.githubusercontent.com/hjdhjd/homebridge-unifi-protect/main/images/homebridge-unifi-protect-4x3.png"));
                        response.write(util.format("tvc-guide-title=\"%s Livestream\" tvc-guide-description=\"UniFi Protect %s %s livestream.\" ", name, camera.marketName, description));
    ...
    devices(model) {
        return Object.values(this.configuredDevices).filter(x => x.ufp?.modelKey === model);
    }
