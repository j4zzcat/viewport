...
import { RtpDemuxer } from "homebridge-plugin-utils";
import { PROTECT_FFMPEG_AUDIO_FILTER_FFTNR, PROTECT_HKSV_FRAGMENT_LENGTH, PROTECT_HKSV_TIMESHIFT_BUFFER_MAXDURATION, PROTECT_HOMEKIT_IDR_INTERVAL, PROTECT_LIVESTREAM_API_IDR_INTERVAL } from "./settings.js";
...
export class ProtectStreamingDelegate {
    ...
    protectCamera;
    probesizeOverride;
    probesizeOverrideCount;
    probesizeOverrideTimeout;
    snapshot;
    verboseFfmpeg;
    abTest = true;
                    ...
                    codecs: [
                        {
                            audioChannels: 1,
                            bitrate: 0,
                            samplerate: [16 /* AudioStreamingSamplerate.KHZ_16 */, 24 /* AudioStreamingSamplerate.KHZ_24 */],
    ...
    async startStream(request, callback) {
        ...
        if (isTranscoding) {
            ...
            rtspEntry ??= this.protectCamera.findRtsp((this.protectCamera.hints.hardwareTranscoding || overrideStream) ? 3840 : request.video.width, (this.protectCamera.hints.hardwareTranscoding || overrideStream) ? 2160 : request.video.height, { biasHigher: true, maxPixels: this.ffmpegOptions.hostSystemMaxPixels });
        ...
        if (!rtspEntry) {
            ...
            this.log.error("%s %sx%s, %s fps, %s kbps.", errorMessage, request.video.width, request.video.height, request.video.fps, request.video.max_bit_rate.toLocaleString("en-US"));
            callback(new Error(this.protectCamera.name + ": " + errorMessage));
            return;
        }
        ...
        const tsBuffer = (this.protectCamera.hints.apiStreaming && this.protectCamera.hints.timeshift) ?
        ...
        if (this.protectCamera.hints.apiStreaming) {
            ...
            ffmpegArgs.push("-probesize", Math.min(this.probesize, tsBuffer?.length ?? this.probesize).toString(), "-f", "mp4", "-i", "pipe:0", "-enc_time_base", "-1", // -1 should be changed to demux for FFmpeg 7 and beyond.
            "-fps_mode", "passthrough", "-muxdelay", "0", "-video_track_timescale", "90000");
        }
        else {
            ...
            ffmpegArgs.push("-probesize", this.probesize.toString(), "-avioflags", "direct", "-rtsp_transport", "tcp", "-i", rtspEntry.url);
        }
        ...
        // Inform the user.
        this.log.info("Streaming request from %s%s: %sx%s@%sfps, %s kbps. %s %s, %s kbps [%s].", sessionInfo.address, (request.audio.packet_time === 60) ? " (high latency connection)" : "", request.video.width, request.video.height, request.video.fps, targetBitrate.toLocaleString("en-US"), isTranscoding ? (this.protectCamera.hints.hardwareTranscoding ? "Hardware accelerated transcoding" : "Transcoding") : "Using", rtspEntry.name, (rtspEntry.channel.bitrate / 1000).toLocaleString("en-US"), this.protectCamera.hints.apiStreaming ? "API" : "RTSP");
        ...
        if (sessionInfo.hasAudioSupport) {
            ...
            ffmpegArgs.push("-map", "0:a:0?", ...this.ffmpegOptions.audioEncoder, "-profile:a", "38", "-flags", "+global_header", "-f", "null", "-ar", request.audio.sample_rate.toString() + "k", "-b:a", request.audio.max_bit_rate.toString() + "k", "-bufsize", (2 * request.audio.max_bit_rate).toString() + "k", "-ac", request.audio.channel.toString(), "-frame_size", (request.audio.packet_time * request.audio.sample_rate).toString());
            ...
            ffmpegArgs.push("-payload_type", request.audio.pt.toString(), "-ssrc", sessionInfo.audioSSRC.toString(), "-f", "rtp", "-srtp_out_suite", "AES_CM_128_HMAC_SHA1_80", "-srtp_out_params", sessionInfo.audioSRTP.toString("base64"), "srtp://" + sessionInfo.address + ":" + sessionInfo.audioPort.toString() + "?rtcpport=" + sessionInfo.audioPort.toString() + "&pkt_size=" +
        ...
        const ffmpegStream = new FfmpegStreamingProcess(this, request.sessionID, ffmpegArgs, (sessionInfo.hasAudioSupport && this.protectCamera.hints.twoWayAudio) ? undefined :
            { addressVersion: sessionInfo.addressVersion, port: sessionInfo.videoReturnPort }, callback);
        if (this.protectCamera.hints.apiStreaming) {
            ...
            const livestreamListener = async (segment) => {
                if (!seenInitSegment) {
                    if (tsBuffer) {
            ...
            const closeListener = () => void ffmpegStream.ffmpegProcess?.stdin.end();
        ...
        const sdpReturnAudio = [
            ...
            "b=AS:24",
            "a=rtpmap:110 MPEG4-GENERIC/" + ((request.audio.sample_rate === 16 /* AudioStreamingSamplerate.KHZ_16 */) ? "16000" : "24000") + "/" + request.audio.channel.toString(),
            "a=fmtp:110 profile-level-id=1;mode=AAC-hbr;sizelength=13;indexlength=3;indexdeltalength=3; config=" +
                ((request.audio.sample_rate === 16 /* AudioStreamingSamplerate.KHZ_16 */) ? "F8F0212C00BC00" : "F8EC212C00BC00"),
    ...
    async handleStreamRequest(request, callback) {
        switch (request.type) {
            ...
            case "reconfigure" /* StreamRequestTypes.RECONFIGURE */:
                ...
                this.log.debug("Streaming parameters adjustment requested by HomeKit: %sx%s, %s fps, %s kbps.", request.video.width, request.video.height, request.video.fps, request.video.max_bit_rate.toLocaleString("en-US"));
                callback();
                break;
