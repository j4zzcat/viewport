...
export class FfmpegRecordingProcess extends FfmpegProcess {
    ...
    constructor(protectCamera, recordingConfig, rtspEntry, isAudioActive) {
        ...
        if (protectCamera.hints.timeshift && protectCamera.stream.hksv?.timeshift.time) {
            ...
            this.commandLineArgs.push("-probesize", (tsBuffer?.length ?? protectCamera.stream.probesize).toString(), "-r", rtspEntry.channel.fps.toString(), "-f", "mp4", "-i", "pipe:0", "-ss", Math.max((protectCamera.stream.hksv?.timeshift.time ?? PROTECT_HKSV_TIMESHIFT_BUFFER_MAXDURATION) - recordingConfig.prebufferLength, 0).toString() + "ms");
        }
        else {
            ...
            this.commandLineArgs.push("-probesize", protectCamera.stream.probesize.toString(), "-r", rtspEntry.channel.fps.toString(), "-rtsp_transport", "tcp", "-i", rtspEntry.url);
        }
        ...
        }), "-fps_mode", "passthrough", "-movflags", "frag_keyframe+empty_moov+default_base_moof", "-reset_timestamps", "1", "-video_track_timescale", "600");
        if (isAudioActive) {
