...
export class ProtectEvents extends EventEmitter {
    ...
    updateUfp(ufp, payload) {
        const mergeJson = (...objects) => {
            const result = {};
    ...
    ufpUpdates(packet) {
        ...
        switch (packet.header.modelKey) {
            ...
            default:
                ...
                if (packet.payload.name && protectDevice.hints.syncName) {
    ...
    motionEventHandler(protectDevice, detectedObjects = [], metadata) {
        if (!protectDevice) {
            return;
        }
    ...
    motionEventDelivery(protectDevice, motionService, detectedObjects = [], metadata) {
        if (!protectDevice) {
            return;
        }
        ...
        if (!protectDevice.hints.smartDetect || (protectDevice.hints.smartDetect && detectedObjects.some(x => protectDevice.hints.smartOccupancy.includes(x)))) {
            ...
            if (occupancyService) {
                ...
                if (occupancyService.getCharacteristic(this.hap.Characteristic.OccupancyDetected).value !== true) {
                    ...
                    // Log the event, if configured to do so.
                    if (protectDevice.hints.logMotion) {
                        protectDevice.log.info("Occupancy detected%s.", protectDevice.hints.smartDetect ? ": " + protectDevice.hints.smartOccupancy.filter(x => detectedObjects.includes(x)).join(", ") : "");
                    }
                }
...
//# sourceMappingURL=protect-events.js.map